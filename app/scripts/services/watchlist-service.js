...
angular.module('stockDogApp')
  .service('WatchlistService', function WatchlistService() {
    ...
    // Argument watchlist with additional helper functions
    var WatchlistModel = {
      addStock: function (stock) {
        var existingSotck = _.find(this.stock, function (s) {
          return s.company.symbol === stock.company.symbol;
        });
        if(existingSotck){
          existingSotck.shares += sotck.shares;
        } else {
    ...
    var loadModel = function () {
      var model = {
        watchlists: localStorage['StockDog.watchlists'] ?
          JSON.parse(localStorage['StockDog.watchlists']): [], nextId: localStorage['StockDog.nextId'] ?
    ...
    var saveModel = function () {
      ...
      localStorage['SotckDog.nextId'] = Model.nextId;
    };
